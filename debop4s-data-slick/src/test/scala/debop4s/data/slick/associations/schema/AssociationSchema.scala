...
trait AssociationSchema {
  ...
  class EmployeeT(tag: Tag) extends IdTable[Employee, Int](tag, "ass_employees") {
  ...
  lazy val Employees = TableQuery[EmployeeT]
  implicit class EmployeeQueryExt(query: TableQuery[EmployeeT]) extends IdTableExtensions[Employee, Int](query)

  class SiteT(tag: Tag) extends IdTable[Site, Int](tag, "ass_site") {
  ...
  lazy val Sites = TableQuery[SiteT]
  implicit class SiteQueryExt(query: TableQuery[SiteT]) extends IdTableExtensions[Site, Int](query)

  class DeviceT(tag: Tag) extends IdTable[Device, Int](tag, "ass_devices") {
    ...
    def site = foreignKey("fk_device_site", siteId, TableQuery[SiteT])(_.id)
  ...
  class OrderT(tag: Tag) extends IdTable[Order, Int](tag, "ass_orders") {
  ...
  lazy val Orders = TableQuery[OrderT]
  implicit class OrderQueryExt(query: TableQuery[OrderT]) extends IdTableExtensions[Order, Int](query)

  class OrderItemT(tag: Tag) extends IdTable[OrderItem, Int](tag, "ass_orderitem") {
    ...
    def order = foreignKey("fk_orderitem_order", orderId, TableQuery[OrderT])(_.id)
    def ixOrder = index("ix_orderitem_order", orderId, unique = false)
  }

  lazy val OrderItems = TableQuery[OrderItemT]
  implicit class OrderItemQueryExt(query: TableQuery[OrderItemT]) extends IdTableExtensions[OrderItem, Int](query)


  class AddressT(tag: Tag) extends IdTable[Address, Int](tag, "ass_address") {
  ...
  lazy val Addresses = TableQuery[AddressT]
  implicit class AddressQueryExt(query: TableQuery[AddressT]) extends IdTableExtensions[Address, Int](query)

  class PersonT(tag: Tag) extends IdTable[Person, Int](tag, "ass_person") {
    ...
    def address = foreignKey("fk_person_address", addressId, TableQuery[AddressT])(_.id)
  ...
  lazy val Persons = TableQuery[PersonT]
  implicit class PersonsQueryExt(query: TableQuery[PersonT]) extends IdTableExtensions[Person, Int](query) {
    def findByAddress(addressId: Int)(implicit session: Session) =
      query.filter(_.addressId === addressId.bind).list
  }

  class TaskT(tag: Tag) extends IdTable[Task, Int](tag, "ass_task") {
  ...
  lazy val Tasks = TableQuery[TaskT]
  implicit class TaskQueryExt(query: TableQuery[TaskT]) extends IdTableExtensions[Task, Int](query)


  class PersonTaskT(tag: Tag) extends Table[PersonTask](tag, "ass_person_task") {
    ...
    def asignee = foreignKey("fk_person_task_person", personId, TableQuery[PersonT])(_.id)
    def task = foreignKey("fk_person_task_task", taskId, TableQuery[TaskT])(_.id)

    def idxPersonTasks = index("ix_person_task", (personId, taskId), unique = true)
  }

  lazy val PersonTasks = TableQuery[PersonTaskT]


  class BankAccountT(tag: Tag) extends IdTable[BankAccount, Int](tag, "ass_bankaccount") {
  ...
  lazy val BankAccounts = TableQuery[BankAccountT]
  implicit class BankAccountQueryExt(query: TableQuery[BankAccountT]) extends IdTableExtensions[BankAccount, Int](query)

  class AccountOwnerT(tag: Tag) extends IdTable[AccountOwner, Int](tag, "ass_bankowner") {
  ...
  lazy val AccountOwners = TableQuery[AccountOwnerT]
  implicit class AccountOwnerQueryExt(query: TableQuery[AccountOwnerT]) extends IdTableExtensions[AccountOwner, Int](query)


  class BankAccountOwnerT(tag: Tag) extends Table[BankAccountOwner](tag, "ass_bankaccount_owner") {
    ...
    def account = foreignKey("fk_bankaccount_owner_account", accountId, TableQuery[BankAccountT])(_.id)
    def owner = foreignKey("fk_bankaccount_owner_owner", ownerId, TableQuery[AccountOwnerT])(_.id)

    def ixBankAccountOwners = index("ix_bankaccount_owner", (accountId, ownerId), unique = true)
  }

  lazy val BankAccountOwners = TableQuery[BankAccountOwnerT]
