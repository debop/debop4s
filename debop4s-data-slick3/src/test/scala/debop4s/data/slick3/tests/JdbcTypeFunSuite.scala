...
class JdbcTypeFunSuite extends AbstractSlickFunSuite {
  ...
  test("byte array") {
    ...
    readonly { ts.result.map(_.map { case (id, data) => (id, data.mkString) }.toSet) } shouldEqual Set((1, "123"), (2, "45"))
    readonly(ts.result).map { case (id, data) => (id, data.mkString) }.toSet shouldBe Set((1, "123"), (2, "45"))
  ...
  test("byte array option") {
    ...
    commit {
      ...
      ifCap(rcap.setByteArrayNull) {
        ts +=(2, None)
      } >>
      ifNotCap(rcap.setByteArrayNull) {
        ts.map(_.id) += 2
      } >>
      ts.result.map(_.map {
        case (id, data) => (id, data.map(_.mkString).getOrElse(""))
      }.toSet).map(_ shouldEqual Set((1, "67"), (2, ""))) >>
      ts.schema.drop
    }
  }
  ...
  test("blob") {
    ...
    val p1: DatabasePublisher[(Int, String)] = db.stream(a1).mapResult {
      case (id: Int, data: Blob) => (id, data.getBytes(1, data.length.toInt).mkString)
    }

    val f1 = materialize(p1).map(_.toSet shouldEqual Set((1, "123"), (2, "45"))) flatMap { p =>
      val f = materializeAsync(db.stream(ts.result.transactionally, bufferNext = false)) {
