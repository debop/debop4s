...
/**
 * debop4s.rediscala.logback log message 를 Redis PubSub channel에서 받아와서 console에 쓰도록 하는 Subscriber 입니다.
 * RedisPubSub 를 사용하는 것이 더 편합니다.
 *
 * Created by debop on 2014. 2. 22.
 */
class ConsoleLogRedisSubscriberActor(override val address: InetSocketAddress,
                                     val channels: Seq[String] = Seq("channel:debop4s.rediscala.logback:logs"),
...
//              channels: Seq[String] = Seq("channel:debop4s.rediscala.logback:logs"),
