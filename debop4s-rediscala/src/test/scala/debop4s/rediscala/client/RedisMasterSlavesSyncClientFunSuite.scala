...
class RedisMasterSlavesSyncClientFunSuite extends FunSuite with Matchers with OptionValues {
  ...
  test("blpop already containing elements") {
    ...
    val (key, msg) = syncRedis.redisBlocking.blpop[String](Seq("blpop1", "blpop2"), 60 seconds).await.get
  ...
  test("blpop blocking") {
    syncRedis.del("blpopBlock")

    val future = syncRedis.redisBlocking.blpop[String](Seq("blpopBlock", "blpopBlock2"), 60 seconds)
    Thread.sleep(100)
  ...
  test("blpop blocking timeout") {
    syncRedis.del("blpopBlockTimeout")

    val future = syncRedis.redisBlocking.blpop[String](Seq("blpopBlockTimeout"), 1 seconds)
  ...
  test("brpop already containing elements") {
    ...
    val future = syncRedis.redisBlocking.brpop[String](Seq("brpop1", "brpop2"), 60 seconds)

    val results = future.await.get
  ...
  test("brpop blocking") {
    syncRedis.del("brpopBlock")

    val future = syncRedis.redisBlocking.brpop[String](Seq("brpopBlock", "brpopBlock2"), 60 seconds)

    Thread.sleep(1000)
    syncRedis.rpush("brpopBlock", "a", "b", "c")

    val results: (String, String) = future.await.get
    results should not be null
  ...
  test("brpop blocking timeout") {
    syncRedis.del("brpopBlockTimeout")
    val future = syncRedis.redisBlocking.brpop[String](Seq("brpopBlockTimeout"), 1 seconds)
  ...
  test("brpoplpush already containing elements") {
    ...
    val value = syncRedis.redisBlocking.brpopplush[String]("brpoplpush1", "brpoplpush2", 60 seconds).await
  ...
  test("brpoplpush blocking") {
    ...
    val future = syncRedis.redisBlocking.brpopplush[String]("brpoplpushBlock1", "brpoplpushBlock2", 60 seconds)

    Thread.sleep(1000)
    syncRedis.rpush("brpoplpushBlock1", "a", "b", "c")

    future.await.get shouldEqual "c"
    syncRedis.del("brpoplpushBlock1", "brpoplpushBlock2")
  }
  ...
  test("brpoplpush blocking timeout") {
    ...
    val future = syncRedis.redisBlocking.brpopplush[String]("brpoplpushTimeout1", "brpoplpushTimeout2", 1 seconds)

    // Timeout 이 되면 None 을 반환합니다.
    future.await.isDefined shouldEqual false
    syncRedis.del("brpoplpushTimeout1", "brpoplpushTimeout2")
  }
  ...
  test("zadd") {
    ...
    syncRedis.zrangeWithScores("zaddKey", 0, -1) should contain allOf(new MemberScore("one", 1.0), new MemberScore("uno", 1.0), new MemberScore("two", 2.0))
    syncRedis.del("zaddKey")
  }
