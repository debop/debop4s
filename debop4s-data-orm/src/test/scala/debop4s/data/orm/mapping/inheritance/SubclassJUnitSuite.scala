...
class SubclassJUnitSuite extends AbstractJpaJUnitSuite {
  ...
  def subclass() {
    ...
    val bankAccount1 = em.find(classOf[SubclassBankAccount], bankAccount.getId)
    assert(bankAccount1 == bankAccount)

    val creditCard1 = em.find(classOf[SubclassCreditCard], creditCard.getId)
    ...
    assert(em.find(classOf[SubclassBankAccount], bankAccount.getId) == null)
    assert(em.find(classOf[SubclassCreditCard], creditCard.getId) == null)
  }
...
abstract class SubclassBillingBase extends LongEntity {
  ...
  @org.hibernate.annotations.Index(name = "ix_subclass_billing_owner", columnNames = Array("owner"))
