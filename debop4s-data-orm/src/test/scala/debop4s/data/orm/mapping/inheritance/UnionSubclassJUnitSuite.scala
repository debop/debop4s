...
import debop4s.data.orm.model.UuidEntity
...
class UnionSubclassJUnitSuite extends AbstractJpaJUnitSuite {
  ...
  def unionSubclass() {
    ...
    assert(bankAccount.getId != null)
    val bankAccount1 = em.find(classOf[UnionSubclassBankAccount], bankAccount.getId)
    assert(bankAccount1 == bankAccount)

    val creditCard1 = em.find(classOf[UnionSubclassCreditCard], creditCard.getId)
    ...
    assert(em.find(classOf[UnionSubclassBankAccount], bankAccount.getId) == null)
    assert(em.find(classOf[UnionSubclassCreditCard], creditCard.getId) == null)
  }
}
