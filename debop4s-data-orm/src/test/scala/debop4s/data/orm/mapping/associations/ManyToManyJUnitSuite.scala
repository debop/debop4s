...
/**
 * ManyToManyJUnitSuite
 *
 * @author sunghyouk.bae@gmail.com
 * @since 2014. 3. 3.
 */
@org.springframework.transaction.annotation.Transactional
class ManyToManyJUnitSuite extends AbstractJpaJUnitSuite {
  ...
  def manyToMany() {
    ...
    assert(soge2.owners.asScala.exists(o => o.id == owner.id))

    em.clear()

    val owner2 = em.find(classOf[AccountOwner], owner.id)
    assert(owner2.bankAccounts.size == 1)
    assert(owner2.bankAccounts.asScala.exists(x => x.id == soge.id))

    val soge3 = owner2.bankAccounts.asScala.head
    ...
    assert(owner3.bankAccounts.asScala.exists(x => x.id == barclays.id))
...
@hba.DynamicInsert
@hba.DynamicUpdate
class AccountOwner extends HibernateEntityBase[java.lang.Long] {
