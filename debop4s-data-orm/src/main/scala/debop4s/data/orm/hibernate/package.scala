...
import debop4s.data.orm.hibernate.utils.{CriteriaUtils, StatelessUtils}
...
package object hibernate {
  implicit class CriteriaExtensions(val criteria: Criteria) {
    ...
    def addBetween(propertyName: String,
      ...
      criteria.add(CriteriaUtils.getBetween(propertyName, lo, hi, includeLo, includeHi))
    }
    ...
    def addInRange(criteria: Criteria,
                   ...
                   includeLo: Boolean,
                   includeHi: Boolean): Criteria = {
      criteria.add(CriteriaUtils.getInRange(loPropertyName, hiPropertyName, value, includeLo, includeHi))
    }
    /**
    ...
    def addOverlap(loPropertyName: String,
                   ...
                   includeLo: Boolean,
                   includeHi: Boolean): Criteria = {
      criteria.add(CriteriaUtils.getOverlap(loPropertyName, hiPropertyName, lo, hi, includeLo, includeHi))
    }
    /** 지정된 시각 이전인 경우 */
    ...
    def addNullAsFalse(propertyName: String, value: Option[Boolean]): Criteria = {
      if (value.isEmpty || value.get)
        ...
        criteria.add(CriteriaUtils.getEqIncludeNull(propertyName, false))
    }
    ...
    def addNullAsTrue(propertyName: String, value: Option[Boolean]): Criteria = {
      ...
      else criteria.add(CriteriaUtils.getEqIncludeNull(propertyName, true))
    }
    ...
    def copy(): Criteria =
      Serializers.copyObject(criteria.asInstanceOf[CriteriaImpl]).asInstanceOf[CriteriaImpl]
    ...
    def setPaging(pageable: Pageable): Criteria = {
      setPaging(pageable.getPageNumber * pageable.getPageSize, pageable.getPageSize)
    }
  ...
  implicit class DetachedCriteriaExtensions(val dc: DetachedCriteria) {
    ...
    def addBetween(propertyName: String,
      ...
      dc.add(CriteriaUtils.getBetween(propertyName, lo, hi, includeLo, includeHi))
    }
    ...
    def addInRange(loPropertyName: String,
                   ...
                   includeLo: Boolean,
                   includeHi: Boolean): DetachedCriteria = {
      dc.add(CriteriaUtils.getInRange(loPropertyName, hiPropertyName, value, includeLo, includeHi))
    }
    ...
    def addOverlap(loPropertyName: String,
                   ...
                   includeLo: Boolean,
                   includeHi: Boolean): DetachedCriteria = {
      dc.add(CriteriaUtils.getOverlap(loPropertyName, hiPropertyName, lo, hi, includeLo, includeHi))
    }
    ...
    def addNullAsFalse(propertyName: String, value: Option[Boolean] = None): DetachedCriteria = {
      ...
      else dc.add(CriteriaUtils.getEqIncludeNull(propertyName, false))
    }
    ...
    def addNullAsTrue(propertyName: String, value: Option[Boolean] = None): DetachedCriteria = {
      ...
      else dc.add(CriteriaUtils.getEqIncludeNull(propertyName, true))
    }
    ...
    def addOrders(orders: java.lang.Iterable[Order]): DetachedCriteria = {
      orders.asScala.foreach { o => dc.addOrder(o) }
      dc
    }
  }
  ...
  implicit class QueryExtensions(val query: Query) {
    ...
    def setPaging(pageable: Pageable): Query = {
      setPaging(pageable.getPageNumber * pageable.getPageSize, pageable.getPageSize)
    }
  ...
  implicit class SessionFactoryExtensions(val sf: SessionFactory) extends Logging {

    def withStateless[T](block: StatelessSession => T): Option[T] = {
      Option(StatelessUtils.withTransaction(sf)(block))
    }

    def withStatelessReadOnly[T](block: StatelessSession => T): Option[T] = {
      Option(StatelessUtils.withReadOnly(sf)(block))
    }
  }
}
