...
/**
 * [[ITimePeriod]] 정보중 시작일자와 완료일자를 Milliseconds 와 TimezoneId 값으로 저장하도록 합니다.
 *
 * {{{
 ...
 */
class TimeRangeAsMillisAndTimeZone extends CompositeUserType {
  ...
  override def nullSafeGet(rs: ResultSet, names: Array[String], session: SessionImplementor, owner: Any): AnyRef = {
    ...
    val endTimestamp = StandardBasicTypes.LONG.nullSafeGet(rs, names(2), session).asInstanceOf[java.lang.Long]
  ...
  override def nullSafeSet(st: PreparedStatement, value: Any, index: Int, session: SessionImplementor): Unit = {
    ...
    if (period == null) {
      ...
      StandardBasicTypes.LONG.nullSafeSet(st, endTimestamp, index + 2, session)
