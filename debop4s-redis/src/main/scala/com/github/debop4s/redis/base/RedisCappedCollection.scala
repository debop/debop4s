...
import com.github.debop4s.redis.serializer.SnappyRedisSerializer
...
class RedisCappedCollection[T](val name: String,
    ...
    */
    ...
    def get(index: Long): Future[T] = {
        ...
        redis.lindex(name, index).map { v => serializer.deserialize(v.get.toArray)}
    }

    def getRange(start: Long, end: Long): Future[Iterable[T]] = {
        redis.lrange(name, start, end).map { list =>
