...
class HourRangeInDay(val start: Timepart,
                     val end: Timepart)
  extends ValueObject with Ordered[HourRangeInDay] {

  def this(startHour: Int, endHour: Int) = this(Timepart(startHour), Timepart(endHour))
  def this(hourOfDay: Int) = this(Timepart(hourOfDay), Timepart(hourOfDay))
