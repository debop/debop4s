...
object PeriodRelation extends Enumeration {
  ...
  /** 알 수 없음 (두개의 기간([[debop4s.timeperiod.TimePeriod]])이 모두 Anytime 일 경우) */
  val NoRelation = Value("NoRelation")

  /** 현 [[debop4s.timeperiod.TimePeriod]] 이후에 대상 [[debop4s.timeperiod.TimePeriod]]가 있을 때 */
  val After = Value("After")

  /** 현 [[debop4s.timeperiod.TimePeriod]]의 완료 시각이 대상 [[debop4s.timeperiod.TimePeriod]]의 시작 시각과 같습니다. */
  val StartTouching = Value("StartTouching")

  /** 현 [[debop4s.timeperiod.TimePeriod]] 기간 안에 대상 [[debop4s.timeperiod.TimePeriod]]의 시작 시각만 포함될 때 */
  val StartInside = Value("StartInside")
  /**
   * 현 [[debop4s.timeperiod.TimePeriod]]의 시작 시각과 대상 [[debop4s.timeperiod.TimePeriod]]의 시작 시각이 일치하고,
   * 대상 [[debop4s.timeperiod.TimePeriod]] 가 현 [[debop4s.timeperiod.TimePeriod]]에 포함될 때
   */
  val InsideStartTouching = Value("InsideStartTouching")
  /**
   * 현 [[debop4s.timeperiod.TimePeriod]]의 시작 시각과 대상 [[debop4s.timeperiod.TimePeriod]]의 시작 시각이 일치하고,
   * 현 [[debop4s.timeperiod.TimePeriod]] 가 대상 [[debop4s.timeperiod.TimePeriod]]에 포함될 때
   */
  val EnclosingStartTouching = Value("EnclosingStartTouching")

  /** 현 [[debop4s.timeperiod.TimePeriod]]가 대상 [[debop4s.timeperiod.TimePeriod]] 기간에 포함될 때 */
  val Enclosing = Value("Enclosing")

  /**
   * 현 [[debop4s.timeperiod.TimePeriod]]의 완료 시각과 대상 [[debop4s.timeperiod.TimePeriod]]의 완료 시각이 일치하고,
   * 현 [[debop4s.timeperiod.TimePeriod]] 가 대상 [[debop4s.timeperiod.TimePeriod]]에 포함될 때
   */
  val EnclosingEndTouching = Value("EnclosingEndTouching")

  /**
   * 현 [[debop4s.timeperiod.TimePeriod]] 기간과 대상 [[debop4s.timeperiod.TimePeriod]]의 기간이 일치할 때,
