...
class CalendarPeriodCollectorFunSuite extends AbstractTimeFunSuite {
  ...
  test("collect years") {
    ...
    collector.periods.foreach { period =>
      ...
      period.isSamePeriod(YearRange(filter.years(i))) shouldEqual true
      i += 1
    }
  }
  ...
  test("collect hours") {
    ...
    collector.periods.size shouldEqual 9
  ...
  test("collect exclude period") {
    ...
    collector3.periods.size shouldEqual workingDays2011 - workingDaysMarch2011 - 2 * WeekDaysPerWeek
