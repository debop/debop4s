package debop4s.timeperiod.calendars.seekers
...
import debop4s.timeperiod.timerange.{DayRange, DayRangeCollection}
 ...
 */
class DaySeekerFunSuite extends AbstractTimeFunSuite {
  ...
  test("min date") {
    val daySeeker = DaySeeker()
    val day = daySeeker.findDay(DayRange(TimeSpec.MinPeriodTime), -10)
    day shouldEqual null
  }

  test("max date") {
    val daySeeker = DaySeeker()
    val day = daySeeker.findDay(DayRange(TimeSpec.MaxPeriodTime), 10)
    day shouldEqual null
  }
