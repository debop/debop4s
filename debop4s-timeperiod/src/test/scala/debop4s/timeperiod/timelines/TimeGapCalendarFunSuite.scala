package debop4s.timeperiod.timelines
 ...
 */
class TimeGapCalendarFunSuite extends AbstractTimeFunSuite {

  val limits = TimeRange(asDate(2011, 3, 1), asDate(2011, 3, 5))
  val calculator = TimeGapCalculator()

  test("no periods") {
    val gaps = calculator.gaps(TimePeriodCollection(), limits)
  ...
  test("get gap") {
    ...
    calculator.gaps(excludePeriods).size should equal(0)
    calculator.gaps(excludePeriods, schoolDay).size should equal(0)
    ...
    val gap2 = calculator.gaps(excludePeriods)
    ...
    val gap3 = calculator.gaps(excludePeriods, testRange3)
    ...
    val gap4 = calculator.gaps(excludePeriods, testRange4)
    ...
    excludePeriods.clear()
    excludePeriods.add(schoolDay.lesson1)
    val gap8 = calculator.gaps(excludePeriods, schoolDay.lesson1)
    ...
    val gap9 = calculator.gaps(excludePeriods, testRange9)
  ...
  test("calendar get gap") {
    ...
    calendars.foreach {
      calendar =>
        ...
        val calculator = TimeGapCalculator(Some(calendar))
        val gaps = calculator.gaps(excludePeriods, limits)

        // gaps.foreach(gap => log.trace(s"$gap"))

        gaps.size should equal(4)
        gaps(0).isSamePeriod(TimeRange(asDate(2011, 3, 4), Durations.days(1))) shouldEqual true
        gaps(1).isSamePeriod(TimeRange(asDate(2011, 3, 9), Durations.days(3))) shouldEqual true
        gaps(2).isSamePeriod(TimeRange(asDate(2011, 3, 14), Durations.days(2))) shouldEqual true
        gaps(3).isSamePeriod(TimeRange(asDate(2011, 3, 18), Durations.days(1))) shouldEqual true
