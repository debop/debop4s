...
import debop4s.timeperiod.samples.TimeBlockPeriodRelationTestData
import debop4s.timeperiod.tests.AbstractTimeFunSuite
import debop4s.timeperiod.utils.{Durations, Times}
import org.joda.time.{DateTime, Duration}

class TimeBlockFunSuite extends AbstractTimeFunSuite {

  val duration = Durations.Hour
  ...
  test("Anytime") {
    assert(TimeBlock.Anytime.start == TimeSpec.MinPeriodTime)
    assert(TimeBlock.Anytime.end == TimeSpec.MaxPeriodTime)
  ...
  test("moment") {
    ...
    assert(block.hasStart)
    assert(block.hasEnd)
    assert(block.duration == TimeSpec.MinDuration)
    assert(!block.isAnytime)
    assert(block.isMoment)
    assert(block.hasPeriod)
  }
  ...
  test("period is not moment") {
    val block = TimeBlock(Times.now, TimeSpec.MinPositiveDuration)
    assert(!block.isMoment)
    assert(block.duration == TimeSpec.MinPositiveDuration)
  }
  ...
  test("start test") {
    val block = TimeBlock(start, start.plusHours(1))
    assert(block.start == start)
  ...
  test("reset") {
    ...
    assert(block.hasEnd)
    block.reset()
    assert(block.start == TimeSpec.MinPeriodTime)
    assert(!block.hasStart)
    assert(block.end == TimeSpec.MaxPeriodTime)
    assert(!block.hasEnd)
  }
