...
import kr.debop4s.timeperiod.timerange.{HalfyearRange, HalfyearRangeCollection}
...
class HalfyearRangeCollectionTest extends AbstractTimePeriodTest {
  ...
  test("single Halfyear") {
    ...
    val halfyearList = halfyears.getHalfyears
    halfyearList.size should equal(1)
    halfyearList(0).isSamePeriod(new HalfyearRange(startYear, startHalfyear)) should equal(true)
  }
