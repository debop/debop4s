...
import com.github.debop4s.timeperiod.DayOfWeek._
...
class TimesTest extends AbstractTimePeriodTest {
    ...
    test("week of year") {
        ...
        foreachDays(period).par.foreach {
            p =>
                val moment = p.start
    ...
    test("foreach years") {
        var count = 0
        Times.foreachYears(period).foreach {
            p =>
    ...
    test("foreach months") {
        var count = 0
        Times.foreachMonths(period).foreach {
            p =>
...
class TimesPeriodTest extends AbstractTimePeriodTest {
    ...
    test("available date time zone") {
        ...
        DateTimeZone.getAvailableIDs.par.foreach {
            id =>
