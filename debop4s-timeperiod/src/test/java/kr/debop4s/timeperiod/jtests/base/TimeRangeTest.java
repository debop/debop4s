...
import kr.debop4s.timeperiod.PeriodRelation;
import kr.debop4s.timeperiod.TimeRange;
import kr.debop4s.timeperiod.jtests.TimePeriodTestBase;
import kr.debop4s.timeperiod.jtests.samples.TimeRangePeriodRelationTestData;
...
import static kr.debop4s.timeperiod.package$.MODULE$;
import static org.fest.assertions.Assertions.assertThat;

/**
 * kr.hconnect.timeperiod.test.base.TimeRangeTest
 *
...
public class TimeRangeTest extends TimePeriodTestBase {
    ...
    private final Duration duration = new Duration(Durations.hours(1, 0, 0, 0));
    ...
    public void anytimeTest() throws Exception {
        assertThat(TimeRange.Anytime().getStart()).isEqualTo(MODULE$.MinPeriodTime());
        assertThat(TimeRange.Anytime().getEnd()).isEqualTo(MODULE$.MaxPeriodTime());
    ...
    public void defaultContructorTest() throws Exception {
        TimeRange range = TimeRange.apply();
    ...
    public void momentTest() throws Exception {
        ...
        TimeRange range = TimeRange.apply(moment);

        assertThat(range.hasStart()).isTrue();
        assertThat(range.hasEnd()).isTrue();
        assertThat(range.getDuration()).isEqualTo(MODULE$.MinDuration());
    ...
    public void momentByPeriod() {
        TimeRange range = TimeRange.apply(Times.now(), Duration.ZERO);
        assertThat(range.isMoment()).isTrue();
    }

    @Test
    public void nonMomentTest() {
        TimeRange range = TimeRange.apply(Times.now(), MODULE$.MinPositiveDuration());
        assertThat(range.isMoment()).isFalse();
        assertThat(range.getDuration()).isEqualTo(MODULE$.MinPositiveDuration());
    }

    @Test
    public void hasStartTest() {
        // 현재부터 ~
        TimeRange range = TimeRange.apply(Times.now(), (DateTime) null);
        assertThat(range.hasStart()).isTrue();
        assertThat(range.hasEnd()).isFalse();
    }
    ...
    public void hasEndTest() {
        //  ~ 현재까지
        TimeRange range = TimeRange.apply(null, Times.now());
        assertThat(range.hasStart()).isFalse();
        assertThat(range.hasEnd()).isTrue();
    }
    ...
    public void startEndTest() {
        TimeRange range = TimeRange.apply(start, end);
    ...
    public void startEndSwapTest() {
        TimeRange range = TimeRange.apply(end, start);
    ...
    public void startAndDurationTest() {
        TimeRange range = TimeRange.apply(start, duration);
    ...
    public void startAndNegateDurationTest() {
        TimeRange range = TimeRange.apply(start, Durations.negate(duration));
    ...
    public void copyConstructorTest() {
        TimeRange source = TimeRange.apply(start, start.plusHours(1), true);
        TimeRange copy = TimeRange.apply(source);
    ...
    public void startTest() {
        TimeRange range = TimeRange.apply(start, start.plusHours(1));
        assertThat(range.getStart()).isEqualTo(start);
    ...
    public void startReadonlyTest() {
        TimeRange range = TimeRange.apply(Times.now(), Durations.hours(1, 0, 0, 0), true);
        range.setStart(range.getStart().minusHours(2));
    }

    @Test(expected = AssertionError.class)
    public void startOutOfRangeTest() {
        TimeRange range = TimeRange.apply(Times.now(), Durations.hours(1, 0, 0, 0), false);
        range.setStart(range.getStart().plusHours(2));
    }

    @Test
    public void endTest() throws Exception {
        TimeRange range = TimeRange.apply(end.minusHours(1), end);
        assertThat(range.getEnd()).isEqualTo(end);
    ...
    public void hasInsidePeriodTest() {
        ...
        // before
        TimeRange before1 = TimeRange.apply(start.minusHours(2), start.minusHours(1));
        TimeRange before2 = TimeRange.apply(start.minusMillis(1), end);
        TimeRange before3 = TimeRange.apply(start.minusMillis(1), start);
        ...
        // after
        TimeRange after1 = TimeRange.apply(start.plusHours(1), end.plusHours(1));
        TimeRange after2 = TimeRange.apply(start, end.plusMillis(1));
        TimeRange after3 = TimeRange.apply(end, end.plusMillis(1));
        ...
        TimeRange inside1 = TimeRange.apply(start.plusMillis(1), end);
        TimeRange inside2 = TimeRange.apply(start.plusMillis(1), end.minusMillis(1));
        TimeRange inside3 = TimeRange.apply(start, end.minusMillis(1));
    ...
    public void copyTest() {
        TimeRange readonlyTimeRange = TimeRange.apply(start, end);
    ...
    public void moveTest() {
        TimeRange moveZero = TimeRange.apply(start, end);
        ...
        TimeRange forward = TimeRange.apply(start, end);
        Duration forwardOffset = Durations.hours(2, 30, 15, 0);
        ...
        TimeRange backward = TimeRange.apply(start, end);
        Duration backwardOffset = Durations.hours(-1, 10, 30, 0);
    ...
    public void expandToPeriodTest() {
        ...
        // no expansion
        range.expandTo(TimeRange.apply(start.plusMillis(1), end.minusMillis(1)));
        assertThat(range.getStart()).isEqualTo(start);
        assertThat(range.getEnd()).isEqualTo(end);

        // start
        DateTime changedStart = start.minusMinutes(1);
        range.expandTo(TimeRange.apply(changedStart, end));
        assertThat(range.getStart()).isEqualTo(changedStart);
        assertThat(range.getEnd()).isEqualTo(end);

        // end
        DateTime changedEnd = end.plusMinutes(1);
        range.expandTo(TimeRange.apply(changedStart, changedEnd));
        assertThat(range.getStart()).isEqualTo(changedStart);
        assertThat(range.getEnd()).isEqualTo(changedEnd);
    ...
    public void shrinkToDateTimeTest() {
        ...
        range = TimeRange.apply(start, end);
    ...
    public void shrinkToPeriodTest() {
        ...
        // no expansion
        range.shrinkTo(TimeRange.apply(start.minusMillis(1), end.plusMillis(1)));
        assertThat(range.getStart()).isEqualTo(start);
        assertThat(range.getEnd()).isEqualTo(end);

        // start
        DateTime changedStart = start.plusMinutes(1);
        range.shrinkTo(TimeRange.apply(changedStart, end));
        assertThat(range.getStart()).isEqualTo(changedStart);
        assertThat(range.getEnd()).isEqualTo(end);

        // end
        DateTime changedEnd = end.minusMinutes(1);
        range.shrinkTo(TimeRange.apply(changedStart, changedEnd));
        assertThat(range.getStart()).isEqualTo(changedStart);
        assertThat(range.getEnd()).isEqualTo(changedEnd);
    ...
    public void isSamePeriodTest() {
        TimeRange range1 = TimeRange.apply(start, end);
        TimeRange range2 = TimeRange.apply(start, end);
    ...
    public void intersectsWithDateTimeTest() {
        ...
        // before
        assertThat(range.intersectsWith(TimeRange.apply(start.minusHours(2), start.minusHours(1)))).isFalse();
        assertThat(range.intersectsWith(TimeRange.apply(start.minusHours(1), start))).isTrue();
        assertThat(range.intersectsWith(TimeRange.apply(start.minusHours(1), start.plusMillis(1)))).isTrue();

        // after
        assertThat(range.intersectsWith(TimeRange.apply(end.plusHours(1), end.plusHours(2)))).isFalse();
        assertThat(range.intersectsWith(TimeRange.apply(end, end.plusMillis(1)))).isTrue();
        assertThat(range.intersectsWith(TimeRange.apply(end.minusMillis(1), end.plusMillis(1)))).isTrue();

        // intersect
        assertThat(range.intersectsWith(range)).isTrue();
        assertThat(range.intersectsWith(TimeRange.apply(start.minusMillis(1), end.plusHours(2)))).isTrue();
        assertThat(range.intersectsWith(TimeRange.apply(start.minusMillis(1), start.plusMillis(1)))).isTrue();
        assertThat(range.intersectsWith(TimeRange.apply(end.minusMillis(1), end.plusMillis(1)))).isTrue();
    }
    ...
    public void getIntersectionTest() {
        ...
        // before
        assertThat(range.getIntersection(TimeRange.apply(start.minusHours(2), start.minusHours(1)))).isNull();
        assertThat(range.getIntersection(TimeRange.apply(start.minusMillis(1), start))).isEqualTo(TimeRange.apply(start));
        assertThat(range.getIntersection(TimeRange.apply(start.minusHours(1), start.plusMillis(1)))).isEqualTo(TimeRange.apply(start, start.plusMillis(1)));

        // after
        assertThat(range.getIntersection(TimeRange.apply(end.plusHours(1), end.plusHours(2)))).isNull();
        assertThat(range.getIntersection(TimeRange.apply(end, end.plusMillis(1)))).isEqualTo(TimeRange.apply(end));
        assertThat(range.getIntersection(TimeRange.apply(end.minusMillis(1), end.plusMillis(1)))).isEqualTo(TimeRange.apply(end.minusMillis(1), end));

        // intersect
        assertThat(range.getIntersection(range)).isEqualTo(range);
        assertThat(range.getIntersection(TimeRange.apply(start.minusMillis(1), end.plusMillis(1)))).isEqualTo(range);
        assertThat(range.getIntersection(TimeRange.apply(start.plusMillis(1), end.minusMillis(1)))).isEqualTo(TimeRange.apply(start.plusMillis(1), end.minusMillis(1)));
    }
    ...
    public void resetTest() {
        ...
        assertThat(range.getStart()).isEqualTo(MODULE$.MinPeriodTime());
        assertThat(range.hasStart()).isFalse();
        assertThat(range.getEnd()).isEqualTo(MODULE$.MaxPeriodTime());
        assertThat(range.hasEnd()).isFalse();
    }
    ...
    public void equalsTest() {
        TimeRange range1 = TimeRange.apply(start, end);
        TimeRange range2 = TimeRange.apply(start, end);
        TimeRange range3 = TimeRange.apply(start.plusMillis(-1), end.plusMillis(1));
        TimeRange range4 = TimeRange.apply(start, end, true);
