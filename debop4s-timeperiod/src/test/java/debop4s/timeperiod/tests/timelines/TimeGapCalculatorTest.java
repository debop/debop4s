package debop4s.timeperiod.tests.timelines;

import debop4s.timeperiod.*;
import debop4s.timeperiod.samples.SchoolDay;
...
public class TimeGapCalculatorTest extends TimePeriodTestBase {
    ...
    public void noPeriods() {
        TimeRange limits = new TimeRange(Times.asDate(2011, 3, 1), Times.asDate(2011, 3, 5));
    ...
    public void periodLargetThanLimits() {
        TimeRange limits = new TimeRange(Times.asDate(2011, 3, 1), Times.asDate(2011, 3, 5));
        TimeGapCalculator calculator = new TimeGapCalculator();
        ITimePeriodCollection excludePeriods = new TimePeriodCollection(new TimeRange(Times.asDate(2011, 2, 1), Times.asDate(2011, 4, 1)));
    ...
    public void periodOutsideLimits() {
        ...
        ITimePeriodCollection excludePeriods =
                new TimePeriodCollection(new TimeRange(Times.asDate(2011, 2, 1), Times.asDate(2011, 2, 5)),
                                         new TimeRange(Times.asDate(2011, 4, 1), Times.asDate(2011, 4, 5)));
    ...
    public void periodOutsideTouchingLimits() {
        ...
        ITimePeriodCollection excludePeriods =
                new TimePeriodCollection(new TimeRange(Times.asDate(2011, 2, 1), Times.asDate(2011, 3, 5)),
                                         new TimeRange(Times.asDate(2011, 3, 20), Times.asDate(2011, 4, 15)));

        ITimePeriodCollection gaps = calculator.getGaps(excludePeriods, limits);
        assertThat(gaps.size()).isEqualTo(1);
        assertThat(gaps.get(0).isSamePeriod(new TimeRange(Times.asDate(2011, 3, 5), Times.asDate(2011, 3, 20)))).isTrue();
    }

    @Test
    public void simpleGaps() {
        TimeRange limits = new TimeRange(Times.asDate(2011, 3, 1), Times.asDate(2011, 3, 20));
        TimeGapCalculator calculator = new TimeGapCalculator();

        TimeRange excludeRange = new TimeRange(Times.asDate(2011, 3, 10), Times.asDate(2011, 3, 15));
    ...
    public void periodTouchingLimitsStart() {
        ...
        ITimePeriodCollection excludePeriods =
                new TimePeriodCollection(new TimeRange(Times.asDate(2011, 3, 1), Times.asDate(2011, 3, 10)));


        ITimePeriodCollection gaps = calculator.getGaps(excludePeriods, limits);
        assertThat(gaps.size()).isEqualTo(1);
        assertThat(gaps.get(0).isSamePeriod(new TimeRange(Times.asDate(2011, 3, 10), Times.asDate(2011, 3, 20)))).isTrue();
    }
    ...
    public void periodTouchingLimitsEnd() {
        ...
        ITimePeriodCollection excludePeriods =
                new TimePeriodCollection(new TimeRange(Times.asDate(2011, 3, 10), Times.asDate(2011, 3, 20)));

        ITimePeriodCollection gaps = calculator.getGaps(excludePeriods, limits);
        assertThat(gaps.size()).isEqualTo(1);
        assertThat(gaps.get(0).isSamePeriod(new TimeRange(Times.asDate(2011, 3, 1), Times.asDate(2011, 3, 10)))).isTrue();
    }
    ...
    public void momentPeriod() {
        ...
        ITimePeriodCollection excludePeriods =
                new TimePeriodCollection(new TimeRange(Times.asDate(2011, 3, 10), Times.asDate(2011, 3, 10)));
    ...
    public void touchingPeriods() {
        TimeRange limits = new TimeRange(Times.asDate(2011, 3, 29), Times.asDate(2011, 4, 1));
        TimeGapCalculator calculator = new TimeGapCalculator();
    ...
    public void calendarGetGap() {
        ...
        for (ITimeCalendar timeCalendar : timeCalendars) {
            ...
            ITimePeriod limits = new CalendarTimeRange(Times.asDate(2011, 3, 4), Times.asDate(2011, 3, 21), timeCalendar);
            ...
            for (ITimePeriod gap : seqAsJavaList(gaps))
                log.trace("gap=[{}]", gap);

            assertThat(gaps.size()).isEqualTo(4);
            assertThat(gaps.get(0).isSamePeriod(new TimeRange(Times.asDate(2011, 3, 4), Durations.days(1)))).isTrue();
            assertThat(gaps.get(1).isSamePeriod(new TimeRange(Times.asDate(2011, 3, 9), Durations.days(3)))).isTrue();
            assertThat(gaps.get(2).isSamePeriod(new TimeRange(Times.asDate(2011, 3, 14), Durations.days(2)))).isTrue();
            assertThat(gaps.get(3).isSamePeriod(new TimeRange(Times.asDate(2011, 3, 18), Durations.days(1)))).isTrue();
        }
    }
}
