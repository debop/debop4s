...
import org.hibernate.cache.rediscala.serializer.{ChillRedisSerializer, FstRedisSerializer, SnappyRedisSerializer, BinaryRedisSerializer}
...
class SerializerTest extends FunSuite with BeforeAndAfter {
    ...
    val kryo = new ChillRedisSerializer[Person]()
    val kryo_snappy = new SnappyRedisSerializer[Person](new ChillRedisSerializer[Person]())
    ...
    test("Chill Serializer benchmark") {
        stopwatch("warn-up", 1) {
            val bytes = kryo.serialize(smallPerson)
            val loaded = kryo.deserialize(bytes)

            val bytes2 = kryo.serialize(largePerson)
            val loaded2 = kryo.deserialize(bytes2)
        }
        ...
        stopwatch("Chill-long", LARGE_LOOP) {
            val bytes = kryo.serialize(largePerson)
            val loaded = kryo.deserialize(bytes)
            assert(largePerson == loaded)
        }
    }
    ...
    test("Chill-Snappy Serializer benchmark") {
        stopwatch("warn-up", 1) {
            val bytes = kryo_snappy.serialize(smallPerson)
            val loaded = kryo_snappy.deserialize(bytes)

            val bytes2 = kryo_snappy.serialize(largePerson)
            val loaded2 = kryo_snappy.deserialize(bytes2)
        }
        ...
        stopwatch("Chill-Snappy-long", LARGE_LOOP) {
            val bytes = kryo_snappy.serialize(largePerson)
            val loaded = kryo_snappy.deserialize(bytes)
            assert(largePerson == loaded)
        }
    }
