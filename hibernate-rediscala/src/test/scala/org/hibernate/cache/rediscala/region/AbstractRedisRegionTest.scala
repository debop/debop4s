...
import org.hibernate.cache.rediscala.domain.{Item, VersionedItem}
 ...
 * @since 2014. 2. 28.
 */
abstract class AbstractRedisRegionTest extends BaseCoreFunctionalTestCase with Matchers {
  ...
  override def configure(cfg: Configuration) {
    ...
    cfg.setProperty(AvailableSettings.CACHE_REGION_PREFIX, "")
  ...
  def queryCacheInvalidation() {
    ...
    item2.name shouldEqual item.name
  ...
  def staleWritesLeaveCacheConsistent() {
    ...
    try {
      ...
      case NonFatal(e) => if (tx2 != null) tx2.rollback()
    } finally {
    ...
    val regionName = RedisCacheUtil.getRegionName(sessionFactory(), classOf[VersionedItem])
    val slcs = sessionFactory().getStatistics.getSecondLevelCacheStatistics(regionName)
