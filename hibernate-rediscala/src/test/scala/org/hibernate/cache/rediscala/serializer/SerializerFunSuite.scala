...
import org.hibernate.cache.rediscala.AbstractHibernateRedisFunSuite
 ...
 */
class SerializerFunSuite extends AbstractHibernateRedisFunSuite {
  ...
  val fst_lz4 = new LZ4RedisSerializer[AnyRef](new FstRedisSerializer[AnyRef]())
  ...
  before {
    ...
    largePerson.lastName = "배성혁"
    largePerson.firstName = "동해물과 백두산이 마르고 닳도록"
  ...
  test("Binary Serializer benchmark") {
    ...
    stopwatch("binary-large", LARGE_LOOP) {
  ...
  test("Snappy Serializer benchmark") {
    ...
    stopwatch("snappy-large", LARGE_LOOP) {
  ...
  test("FST Serializer benchmark") {
    ...
    stopwatch("FST-large", LARGE_LOOP) {
  ...
  test("FST-Snappy Serializer benchmark") {
    ...
    stopwatch("FST-Snappy-large", LARGE_LOOP) {
  ...
  test("FST-LZ4 Serializer benchmark") {
    stopwatch("warn-up", 1) {
      val bytes = fst_lz4.serialize(smallPerson)
      val loaded = fst_lz4.deserialize(bytes)

      val bytes2 = fst_lz4.serialize(largePerson)
      val loaded2 = fst_lz4.deserialize(bytes2)
    }

    stopwatch("FST-LZ4-small", SMALL_LOOP) {
      val bytes = fst_lz4.serialize(smallPerson)
      val loaded = fst_lz4.deserialize(bytes)
      assert(smallPerson == loaded)
    }

    stopwatch("FST-LZ4-large", LARGE_LOOP) {
      val bytes = fst_lz4.serialize(largePerson)
      val loaded = fst_lz4.deserialize(bytes)
      assert(largePerson == loaded)
    }
  }
