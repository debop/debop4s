...
abstract class AbstractObjectPool[T <: AnyRef](val config: GenericObjectPoolConfig,
   ...
   */
  def returnResource(resource: T): Unit =
   ...
   * @param resource 반환할 리소스
   */
  protected def returnResourceObject(resource: T): Unit = {
   ...
   **/
  def returnBrokenResource(broken: T): Unit = {
   ...
   * @param broken 재사용이 불가한 리소스
   **/
  protected def returnBrokenResourceObject(broken: T): Unit = {
