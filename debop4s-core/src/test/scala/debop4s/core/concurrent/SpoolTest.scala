 ...
 */
class SpoolTest extends AbstractCoreTest {
    ...
    test("Empty Spool - append via ++") {
        ...
        Asyncs.result(s2.toSeq) should contain only(3, 4)
    }
    ...
    test("Empty Spool - append via ++ with Future rhs") {
        ...
        Asyncs.result(s2 flatMap (_.toSeq)) should contain only(3, 4)
    }
    ...
    test("Simple Spool - iterate over all elements") {
        ...
        xs.toSeq should contain allOf(1, 2)
    }

    test("Simple Spool - buffer to a sequence") {
        Asyncs.result(simpleSpool.toSeq) should contain only(1, 2)
    }

    test("Simple Spool - map") {
        Asyncs.result(simpleSpool map { _ * 2 } toSeq) should contain only(2, 4)
    }
    ...
    test("Simple Spool - append via ++") {
        Asyncs.result((simpleSpool ++ Spool.empty[Int]).toSeq) should contain only(1, 2)
        Asyncs.result((Spool.empty[Int] ++ simpleSpool).toSeq) should contain only(1, 2)

        val s2 = simpleSpool ++ (3 **:: 4 **:: Spool.empty[Int])
        Asyncs.result(s2.toSeq) should be(Seq(1, 2, 3, 4))
    }

    test("Simple Spool - append via ++ with Future rhs") {
        Asyncs.result(simpleSpool ++# Future(Spool.empty[Int]) flatMap (_.toSeq)) should equal(Seq(1, 2))
        Asyncs.result(Spool.empty[Int] ++# Future(simpleSpool) flatMap (_.toSeq)) should equal(Seq(1, 2))

        val s2 = simpleSpool ++# Future(3 **:: 4 **:: Spool.empty[Int])
        Asyncs.result(s2 flatMap (_.toSeq)) should equal(Seq(1, 2, 3, 4))
    }

    test("Simple Spool - flatMap") {
        val f = (x: Int) => Future(x.toString **:: (x * 2).toString **:: Spool.empty[String])
        val s2 = simpleSpool flatMap f
        Asyncs.result(s2 flatMap (_.toSeq)) should equal(Seq("1", "2", "2", "4"))
    }
    ...
    test("be roundtrippable through toSeq/toSpool") {
        val seq = (0 to 10).seq
        Asyncs.result(seq.toSpool.toSeq) should equal(seq)
    }
    ...
    test("Simple Spool flatten via flatMap of toSpool") {
        ...
        Asyncs.result(flatSpool.flatMap(_.toSeq)) should equal(seq.flatten)
    }
    ...
    test("Simple delayed Spool") {
        ...
        s foreach { xs += _ }
        xs.toSeq should contain only 1
    ...
    test("EOF iteration on EOFException") {
        ...
        s foreachElem { xs += _ }
        xs.toSeq should contain only Some(1)
        p failure new EOFException("sad panda")
        xs.toSeq should contain allOf(Some(1), None)
    }
    ...
    test("return a buffered seq when complete") {
        ...
        Asyncs.result(f) should contain only(1, 2)
    ...
    test("collect") {
        ...
        val sls = s1.toSeq
        Asyncs.result(sls) should contain only(4, 8)
    }
